...
def mod_gen(from_race, from_gender, from_voice):
    ...
    mod_directory = f"./CURSED/Lala curse for {from_race} {from_gender} {from_voice +1}"
    meta_filename = mod_directory + "/meta.json"
    meta_file = (
        """{
  "FileVersion": 1,
  "Name": "Lala fragrance for """
...
def groups_gen(from_race, from_gender, from_voice):
    ...
    for to_emote in range(0, len(emote_names)):
        group += 1
        filename = mod_directory + f"/group_{group:03d}_{emote_names[to_emote]}.json"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        with open(filename, "w") as file:
            file.write(group_gen(from_race, from_gender, from_voice, to_emote))
    pass


def group_gen(from_race, from_gender, from_voice, to_emote):
    global prio
    prio += 1
    offset_a = (from_voice % 3) * 1000
    offset_b = (offsets[from_race] - offsets["Lalafell"])*6000
    from_id = voice_ids[from_race][from_gender][from_voice] + offset_a - offset_b
    if from_race == "Lalafell":
        from_id = voice_ids[from_race][from_gender][from_voice]

    print(f"creating group emote {to_emote} from_id: {from_id} offsets {offset_a} {offset_b}")
    return (
        """ {
    "Name": \"Lalafell """
        + str(emote_names[to_emote])
        + """ voice\",
    ...
    "DefaultSettings": 0,
    "Options": ["""
        + options_gen(to_emote, from_id)
        + """]
    }"""
    )


def options_gen(emote, from_id):
    global prio
    tmp = [
        """{
      "Name": "None",
    ...
    for gender in voice_ids["Lalafell"].keys():
        for voice in range(0, len(voice_ids["Lalafell"][gender])):
            tmp.append(option_gen(gender, voice, from_id, emote))
    return ",\n".join(tmp)


def option_gen(gender, voice, from_id, emote_id):
    global prio
    prio += 1
    tmp = file_swap(from_id  + emote_id, voice_ids["Lalafell"][gender][voice] + emote_id)
...
if __name__ == "__main__":
    ...
    #print(group_gen("Hrothgar", "Masculine", 6, 1 ))
    #print(options_gen( 1, 17311000))
    #print(option_gen( "Masculine", 1, 17311000, 1))
    #print(file_swaps( 17311000, 1))
