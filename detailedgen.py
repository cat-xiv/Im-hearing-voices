...
def mod_gen(from_race, from_gender, from_voice):
    ...
    mod_directory = f"./DetailedChoice/Voice for {from_race} {from_gender} {from_voice +1}"
    meta_filename = mod_directory + "/meta.json"
    meta_file = (
        """{
  "FileVersion": 1,
  "Name": "Voice Change for """
...
def groups_gen(from_race, from_gender, from_voice):
    ...
    for to_race in voice_ids.keys():
        for to_emote in range(0, len(emote_names)):
            group += 1
            filename = mod_directory + f"/group_{group:03d}_{to_race}_{emote_names[to_emote]}.json"
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            with open(filename, "w") as file:
                file.write(group_gen(from_race, from_gender, from_voice, to_race, to_emote))
    pass


def group_gen(from_race, from_gender, from_voice, to_race, to_emote):
    global prio
    prio += 1
    ...
    "DefaultSettings": 0,
    "Options": ["""
        + options_gen(to_emote,to_race, from_id)
        + """]
    }"""
    )


def options_gen(emote,to_race, from_id):
    global prio
    tmp = [
        """{
      "Name": "None",
    ...
    for gender in voice_ids[to_race].keys():
        for voice in range(0, len(voice_ids[to_race][gender])):
            tmp.append(option_gen(gender, voice, from_id, emote, to_race))
    return ",\n".join(tmp)


def option_gen(gender, voice, from_id, emote_id , to_race):
    global prio
    prio += 1
    tmp = file_swap(from_id  + emote_id, voice_ids[to_race][gender][voice] + emote_id)
